...
import { Plus, Trash2, Download, MoreVertical, RotateCcw, ChevronDown, AlertTriangle, X, User, MapPin, Calendar, Loader2 } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useMutation, useQueryClient } from '@tanstack/react-query';
import api from '@/lib/api/client';
import type { Client, NEMTRequest, NEMTBroker, NEMTRequestFormData, NEMTServiceCheckboxes, NEMTRecurringDays, PaginatedNEMTRequests } from '@/types';
...
const getDefaultFormData = (client: Client): NEMTRequestFormData => {
  ...
  const fullAddress = address
    ? `${address.street || ''}, ${address.city || ''}, ${address.state || ''} ${address.zip || ''}`.replace(/, ,/g, ',').replace(/^, /, '')
    : '';
  ...
  return {
    ...
    assessor_name: client.case_manager?.user?.name || client.case_manager?.name || '',
    ...
    is_recurring: false,
    special_needs: `\n\nCall ${client.owner_name || ''} ${client.tenant_phone || ''} before no show.`,
...
export default function NEMTRequestModal({
  ...
  const [formData, setFormData] = useState<NEMTRequestFormData>(() => getDefaultFormData(client));
  const [errors, setErrors] = useState<FormErrors>({});
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [showAdditionalOptions, setShowAdditionalOptions] = useState(false);
  const [timeWarnings, setTimeWarnings] = useState<{ message: string; type: 'warning' | 'error' | '' }>({ message: '', type: '' });
  ...
  const canCreateRequest = useMemo(() => {
    return (client.available_nemt_brokers?.length ?? 0) > 0 &&
           !client.is_inactive &&
  ...
  const validateTimeRange = useCallback(() => {
    ...
    if (start >= end) {
      setTimeWarnings({ message: 'End time must be after start time', type: 'error' });
      return;
    }
    ...
    if (durationMinutes > 480) {
      setTimeWarnings({ message: 'Appointment duration exceeds 8 hours. Please verify times.', type: 'warning' });
      return;
    }
  ...
  const updateField = <K extends keyof NEMTRequestFormData>(field: K, value: NEMTRequestFormData[K]) => {
  ...
  const updateServiceCheckbox = (key: keyof NEMTServiceCheckboxes, value: boolean) => {
    setFormData(prev => ({
      ...prev,
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.assessor_name) newErrors.assessor_name = 'Assessor name is required';
    if (!formData.assessor_phone) newErrors.assessor_phone = 'Assessor phone is required';
    ...
    if (!formData.dropoff_address) newErrors.dropoff_address = 'Dropoff address is required';
    if (!formData.appointment_start_time) newErrors.appointment_start_time = 'Start time is required';
    if (!formData.appointment_end_time) newErrors.appointment_end_time = 'End time is required';
    if (formData.appointment_start_time && formData.appointment_end_time &&
        formData.appointment_start_time >= formData.appointment_end_time) {
      newErrors.appointment_end_time = 'End time must be after start time';
    }
    if (!formData.transportation_date) newErrors.transportation_date = 'Date is required';

    if (formData.is_recurring) {
      if (!formData.end_date) newErrors.end_date = 'End date is required for recurring appointments';
      const hasSelectedDays = Object.values(formData.recurring_days).some(day => day);
      if (!hasSelectedDays) newErrors.recurring_days = 'Please select at least one day of week';
    }
  ...
  const createMutation = useMutation({
    mutationFn: async (data: NEMTRequestFormData) => {
      const response = await api.post<{ success: boolean; pdf_url?: string; message?: string }>(
        '/tenant-api/nemt-requests',
        data
      );
    ...
    onError: (error: Error & { response?: { data?: { errors?: FormErrors; message?: string } } }) => {
      if (error.response?.data?.errors) {
        ...
        setErrors({ general: error.response?.data?.message || error.message || 'Failed to create transportation request' });
      }
    },
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (client.is_inactive) {
      setErrors({ general: 'This client is inactive. Transportation requests cannot be created.' });
      return;
    }

    const missingData = getMissingClientData();
    if (missingData.length > 0) {
      setErrors({ general: `Cannot create transportation request. Missing required information:\n\n• ${missingData.join('\n• ')}\n\nPlease update the client profile and broker assignments first.` });
      return;
    }
    ...
    const submitData = {
      ...formData,
      end_date: formData.is_recurring ? formData.end_date : formData.transportation_date,
    };
  ...
  const updateRecurringSchedule = () => {
    ...
    if (selectedDays && formData.transportation_date && formData.end_date) {
      ...
      if (!isNaN(start.getTime()) && !isNaN(end.getTime())) {
        ...
        const baseText = formData.special_needs.split('***Please schedule every')[0].trim();

        updateField('special_needs', `${baseText}\n\n***Please schedule every ${selectedDays} through ${monthsList}, ${year}.***`);
      }
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {!canCreateRequest && !client.is_inactive && missingData.length > 0 && (
          <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg">
            <div className="flex items-start">
              ...
              <div>
                <p className="text-sm font-medium text-yellow-800 dark:text-yellow-400">Cannot create transportation requests</p>
                <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">Missing required information:</p>
        ...
        {errors.general && (
          <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-lg">
            <div className="flex items-start">
              ...
              <div>
                <p className="text-sm font-medium text-red-800 dark:text-red-400">Unable to Create Transportation Request</p>
                <p className="mt-1 text-sm text-red-700 dark:text-red-300 whitespace-pre-line">{errors.general}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {previousRequests.length > 0 && (
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">Click to fill the form with a previous request</p>
        ...
        {/* Broker Information */}
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">NEMT Broker</label>
          {client.nemt_broker ? (
            <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-md">
              <div className="text-sm text-gray-900 dark:text-gray-100">{client.nemt_broker.name}</div>
              <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                {client.nemt_broker.fax && `Fax: ${client.nemt_broker.fax}`}
                {client.nemt_broker.dda_region && (client.nemt_broker.fax ? ` • Region: ${client.nemt_broker.dda_region}` : `Region: ${client.nemt_broker.dda_region}`)}
              </div>
            </div>
          ) : (
            <p className="text-sm text-red-600 dark:text-red-400">No broker assigned. Please assign a broker in the NEMT Broker Management section.</p>
          )}
          {errors.broker_name && <p className="mt-1 text-sm text-red-600">{errors.broker_name}</p>}
        </div>

        {/* Basic Information Section */}
        <div className="border-b border-gray-200 dark:border-gray-700 pb-2">
          <h3 className="text-lg font-medium text-gray-500 dark:text-gray-400">Basic Information</h3>
        </div>
          ...
          <div>
            ...
            <input
              ...
              className={cn(
                ...
                errors.assessor_name ? 'border-red-300' : 'border-gray-300 dark:border-gray-600'
              )}
              required
            />
            {errors.assessor_name && <p className="mt-1 text-sm text-red-600">{errors.assessor_name}</p>}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Assessor Phone</label>
            <input
              type="text"
              value={formData.assessor_phone}
              onChange={(e) => updateField('assessor_phone', formatPhoneNumber(e.target.value))}
              placeholder="(555) 555-5555"
              maxLength={14}
              className={cn(
                'w-full px-3 py-2 border rounded-md shadow-sm focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-white text-sm',
                errors.assessor_phone ? 'border-red-300' : 'border-gray-300 dark:border-gray-600'
              )}
              required
            />
            {errors.assessor_phone && <p className="mt-1 text-sm text-red-600">{errors.assessor_phone}</p>}
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Mobility Status</label>
          <select
            value={formData.mobility_status}
            onChange={(e) => updateField('mobility_status', e.target.value as NEMTRequestFormData['mobility_status'])}
            className={cn(
              'w-full px-3 py-2 border rounded-md shadow-sm focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-white text-sm',
              errors.mobility_status ? 'border-red-300' : 'border-gray-300 dark:border-gray-600'
            )}
          >
          ...
          {errors.mobility_status && <p className="mt-1 text-sm text-red-600">{errors.mobility_status}</p>}
        </div>

        {/* Address and Times Section */}
        <div className="border-b border-gray-200 dark:border-gray-700 pb-2">
          <h3 className="text-lg font-medium text-gray-500 dark:text-gray-400">Address and Times</h3>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className={cn(
                'w-full px-3 py-2 border rounded-md shadow-sm focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-white text-sm',
                errors.dropoff_address ? 'border-red-300' : 'border-gray-300 dark:border-gray-600'
              )}
              required
            />
            {errors.dropoff_address && <p className="mt-1 text-sm text-red-600">{errors.dropoff_address}</p>}
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Appointment Start Time</label>
            <input
              type="time"
              value={formData.appointment_start_time}
              onChange={(e) => updateField('appointment_start_time', e.target.value)}
              className={cn(
                'w-full px-3 py-2 border rounded-md shadow-sm focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-white text-sm',
                errors.appointment_start_time || timeWarnings.type === 'error' ? 'border-red-300' :
                timeWarnings.type === 'warning' ? 'border-yellow-300' : 'border-gray-300 dark:border-gray-600'
              )}
              required
            />
            {errors.appointment_start_time && <p className="mt-1 text-sm text-red-600">{errors.appointment_start_time}</p>}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Appointment End Time</label>
            <input
              type="time"
              value={formData.appointment_end_time}
              onChange={(e) => updateField('appointment_end_time', e.target.value)}
              className={cn(
                'w-full px-3 py-2 border rounded-md shadow-sm focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-white text-sm',
                errors.appointment_end_time || timeWarnings.type === 'error' ? 'border-red-300' :
                timeWarnings.type === 'warning' ? 'border-yellow-300' : 'border-gray-300 dark:border-gray-600'
              )}
              required
            />
            {errors.appointment_end_time && <p className="mt-1 text-sm text-red-600">{errors.appointment_end_time}</p>}
          </div>
        </div>
        ...
        <div className="border-b border-gray-200 dark:border-gray-700 pb-2">
          <h3 className="text-lg font-medium text-gray-500 dark:text-gray-400">Transportation Dates</h3>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className={cn(
                'w-full px-3 py-2 border rounded-md shadow-sm focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-white text-sm',
                errors.transportation_date ? 'border-red-300' : 'border-gray-300 dark:border-gray-600'
              )}
              required
            />
            {errors.transportation_date && <p className="mt-1 text-sm text-red-600">{errors.transportation_date}</p>}
          </div>
          ...
          <div className="space-y-2 mt-6">
            <div className="flex items-center">
              <input
                ...
                onChange={(e) => updateField('support_person_rides', e.target.checked)}
                className="h-4 w-4 text-violet-600 focus:ring-violet-500 border-gray-300 rounded"
              />
              <label htmlFor="support_person_rides" className="ml-2 text-sm text-gray-700 dark:text-gray-300">
        ...
        {formData.is_recurring && (
          <div>
            ...
            <input
              ...
              className={cn(
                'w-full px-3 py-2 border rounded-md shadow-sm focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-white text-sm',
                errors.end_date ? 'border-red-300' : 'border-gray-300 dark:border-gray-600'
              )}
              required
            />
        ...
        {formData.is_recurring && (
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select Days of Week</label>
                ...
                <div key={day} className="flex items-center">
                  <input
                    ...
                    className={cn(
                      ...
                      errors.recurring_days ? 'border-red-300' : 'border-gray-300'
                    )}
                  />
                  <label htmlFor={day} className="ml-2 text-sm text-gray-700 dark:text-gray-300 capitalize">
                    {day}
                  </label>
                </div>
              ))}
            </div>
            {errors.recurring_days && <p className="mt-1 text-sm text-red-600">{errors.recurring_days}</p>}
        ...
        <div className="border-b border-gray-200 dark:border-gray-700 pb-2">
          <h3 className="text-lg font-medium text-gray-500 dark:text-gray-400">Special Needs & Comments</h3>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Special Needs / Comments</label>
          <textarea
        ...
        <div className="border-b border-gray-200 dark:border-gray-700 pb-2">
          <h3 className="text-lg font-medium text-gray-500 dark:text-gray-400">Additional Options</h3>
        </div>
        ...
        <div>
          ...
          >
            <ChevronDown className={cn('w-5 h-5 mr-2 transition-transform', showAdditionalOptions && 'rotate-180')} />
            Additional Options
          </button>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center">
                    ...
                    />
                    <label className="ml-2 text-sm text-gray-700 dark:text-gray-300">Client is Medicaid Eligible</label>
                  </div>
                  <div className="flex items-center">
                    <input
                      type="checkbox"
                    ...
                    <label className="ml-2 text-sm text-gray-700 dark:text-gray-300">
                      Client needs transportation to an alternate location to receive PASRR Specialized Add-on Services
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    { key: 'staff_family_consultation', label: 'Staff / family consultation' },
                    { key: 'community_access', label: 'Community Access' },
                    { key: 'supported_employment', label: 'Supported employment services' },
                    { key: 'community_guide', label: 'Community Guide' },
                    { key: 'transportation', label: 'Transportation' },
                    { key: 'habilitative_behavior', label: 'Habilitative behavior support and consultation' },
                    { key: 'habilitative_therapy', label: 'Habilitative therapy services' },
                  ].map(({ key, label }) => (
                    <div key={key} className="flex items-center">
                      <input
                        type="checkbox"
                        checked={formData.service_checkboxes[key as keyof NEMTServiceCheckboxes] || false}
                        onChange={(e) => updateServiceCheckbox(key as keyof NEMTServiceCheckboxes, e.target.checked)}
                        className="h-4 w-4 text-violet-600 focus:ring-violet-500 border-gray-300 rounded"
                      />
                      <label className="ml-2 text-sm text-gray-700 dark:text-gray-300">{label}</label>
                    </div>
                  ))}
                  <div className="flex items-center col-span-2">
                    <input
                      type="checkbox"
                      checked={formData.service_checkboxes.other_habilitative || false}
                      onChange={(e) => updateServiceCheckbox('other_habilitative', e.target.checked)}
                      className="h-4 w-4 text-violet-600 focus:ring-violet-500 border-gray-300 rounded"
                    />
                    <label className="ml-2 text-sm text-gray-700 dark:text-gray-300">
